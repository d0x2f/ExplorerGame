...
void main() {
	...
	vec4 diffuse = vec4(0.3, 0.3, 0.3, 1.0) * max(dot(normal,L), 0.0);
	vec4 specular = vec4(0.1, 0.1, 0.1, 1.0) * pow(max(dot(R,E),0.0), 60);
	
	vec4 tex = texture(tex0, tex_coords);
	out_colour += tex*(1-((vertex.z+0.4)*(1/0.4)));
	
	tex = texture(tex1, tex_coords);
	out_colour += tex*((vertex.z+0.4)*(1/0.4));
